...
\begin{frame}{Visualising the Proximal Operator}
    ...
    \begin{tikzpicture}[scale=1]
        % Convex set
        \draw[fill=blue!10, draw=blue!50, thick] plot[smooth cycle] coordinates {(0,0) (2.5,0.3) (3,2) (2,2.8) (0.7,2.3)};
        \node[blue!70, font=\small] at (1.5, 1.3) {Level sets of $J(u)$};
        
        % Point v outside
        \node[circle, fill=red, inner sep=2.5pt, label=above right:{$v$}] (v) at (4.5, 3.2) {};
        
        % Proximal point
        \node[circle, fill=darkgreen, inner sep=2.5pt, label=below right:{$\operatorname{prox}_J(v)$}] (prox) at (2.6, 2.4) {};
        
        % Distance line
        \draw[thick, red!60, dashed] (v) -- (prox);
        \node[above, rotate=-28, font=\footnotesize, red!70] at (3.55, 2.8) {minimize $\|u - v\|^2$};
        
        % Contour circles around prox point
        \foreach \r in {0.6, 1.2, 1.8} {
            \draw[gray!30, thin] (prox) circle (\r);
        }
        
        % Gradient-like arrow showing balance
        \draw[->, thick, blue!70] (prox) -- ++(0.5, -0.3) node[right, font=\tiny] {$\partial J$};
        \draw[->, thick, red!70] (prox) -- ++(0.8, 0.35) node[above, font=\tiny] {$v - u$};
        
        \node[below, font=\footnotesize] at (2.2, -0.5) {The proximal operator balances proximity to $v$ and minimising $J(u)$};
    \end{tikzpicture}
    \end{center}
\end{frame}
...
\begin{frame}{Visualising the Quadratic Approximation}
    ...
    \begin{tikzpicture}[scale=0.9]
        ...
        % Original smooth L(u)
        \addplot[red, very thick, domain=0:4, samples=50] {0.25*(x-1.8)^2 + 0.6};
        \addlegendentry{$L(u)$ (smooth)}
        
        % Quadratic approximation at u^k
        \addplot[blue, dashed, thick, domain=0.2:3.5, samples=50] {0.5*(x-1.2)^2 + 0.6};
        \addlegendentry{Quadratic model at $u^k$}
        
        % Non-smooth S(u)
        \addplot[darkgreen, very thick, domain=0:4, samples=50] {0.35*abs(x-1.8) + 0.2};
        \addlegendentry{$S(u)$ (non-smooth)}
        
        % Combined objective (schematic)
        \addplot[orange, thick, densely dotted, domain=0.5:3.5, samples=50] {0.5*(x-1.2)^2 + 0.35*abs(x-1.8) + 0.8};
        \addlegendentry{$L_{\text{quad}} + S(u)$}
        
        % Mark points
        \node[circle, fill=black, inner sep=2pt] at (axis cs:1.2,0.6) {};
        \node[circle, fill=black, inner sep=2pt] at (axis cs:2.5,0.95) {};
        
        % Arrow showing minimization
        \draw[->, thick, purple] (axis cs:1.2, 1.8) -- (axis cs:2.5, 1.5);
        \node[purple, font=\small] at (axis cs:1.85, 2.2) {minimize approximation};
        \end{axis}
    \end{tikzpicture}
    \end{center}
    
    \vspace{-0.2cm}
    The proximal step minimizes the \emph{quadratic model + non-smooth term} exactly.
\end{frame}
