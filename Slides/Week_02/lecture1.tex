...
A regularisation operator $R_\alpha$ together with a \textbf{parameter choice strategy} $\alpha_{\text{choice}}: (0, \delta_0) \times \mathcal{V} \to \mathbb{R}_{>0}$, denoted by $\alpha(\delta, f^\delta)$, forms a \textbf{regularisation method}.
\end{definition}
...
A spectral operator $R_\alpha$ is bounded if and only if the filter functions $g_\alpha(\sigma)$ are uniformly bounded for a fixed $\alpha$:
\begin{align*}
...
\begin{frame}
\frametitle{Tikhonov: Operator Form}
...
\begin{itemize}
    ...
    \item This is also known as \textbf{Ridge Regression} in statistics.
\end{itemize}
\end{frame}
