...
\begin{frame}{Recap: Algorithm Unrolling}
    In the previous chapter, we explored the paradigm of \textbf{algorithm unrolling}:
...
\begin{frame}{The Perspective Shift}
    ...
    \textbf{The Solution:} We initiate a profound perspective shift.
    \begin{itemize}
...
\begin{frame}{The Exploding/Vanishing Gradient Problem}
    ...
    Mathematically, the gradient of the transition must be close to the identity matrix:
    $$\nabla G\nabla h\sim I$$
...
\begin{frame}{ResNets as Stable Additive Perturbations}
    \textbf{Residual Networks (ResNets)} naturally fulfil this stability condition.\vspace{1em}
    ...
    This ensures that $\nabla_{u^l}u^{l+1}=I+\nabla_{u^l}\mathcal{F}(u^l,\Theta_l)$, strongly regularising the flow of gradients and allowing for the training of networks with hundreds of layers.
\end{frame}
...
\begin{frame}{Overcoming Numerical Instability}
    ...
    \begin{itemize}
        ...
        \item This continuous perspective systematically overcomes the numerical instabilities inherent in standard learning algorithms.
    \end{itemize}
\end{frame}
...
\begin{frame}{Transforming Supervised Learning}
    By formally interpreting the neural network as a continuous dynamical system, the supervised training process is mathematically transformed into an \textbf{optimal control problem}.
    
    \vspace{1em}
    Instead of finding a discrete set of matrices to minimise a sum, our objective is to find an \emph{optimal continuous control trajectory} $\Theta(t)$ that steers the dynamical system to a desired terminal state at time $t=T$.
\end{frame}
...
\begin{frame}{Example: Training a Classifier on Data}
    Suppose we have a dataset of $m$ input-output pairs $(x_i, y_i) \in \mathbb{R}^d \times \mathbb{R}$. We construct the terminal cost function $\mathcal{M}$ (empirical risk) and a running cost $\mathcal{R}$ (Tikhonov regularisation):
...
\begin{frame}{Gradient-Based Optimisation}
    ...
    Rather than calculating this via cumbersome standard variational calculus and chain rules, we employ \textbf{Lagrange multipliers}. 
    \vspace{1em}
...
\begin{frame}{The Adjoint State $p(t)$}
    We introduce a continuous-time Lagrange multiplier $p(t) \in \mathcal{U}$, referred to in optimal control terminology as the \textbf{adjoint state} (or co-state). \vspace{1em}
...
\begin{frame}{Integration by Parts}
    We isolate $u(t)$ by applying integration by parts to the first half of the constraint integral:
...
\begin{frame}{First-Order Optimality: The Adjoint Equation}
    Taking the variation with respect to the state variable $u$ yields the adjoint equation. At the terminal boundary $u(T)$:
    $$\nabla_{u(T)}\mathcal{L}=0\quad\implies\quad p(T)=\nabla_u\mathcal{M}\left(u(T)\right)$$
    
    For the continuous interval $(0, T)$:
