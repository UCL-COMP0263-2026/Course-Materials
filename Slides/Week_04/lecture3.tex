...
\begin{frame}{Recap: The Analysis Perspective}
    In the previous lecture (Block 2), we focused on \textbf{Analysis Regularisation} (specifically NETT).
...
\begin{frame}{Learning the Critic via Wasserstein Distance}
    ...
    We use the \textbf{Wasserstein-1 Distance} (Earth Mover's Distance). By Kantorovich-Rubinstein duality:
...
\begin{frame}{Enforcing 1-Lipschitz Constraints}
    ...
    $\hat{u}$ are samples interpolated between real and fake images.
\end{frame}

\begin{frame}{The Variational Step}
    Once the Critic $J_\Theta^*$ is trained, it serves as our regulariser.
    
    For a new noisy measurement $f^\delta$, we solve:
    \begin{align}
...
\begin{frame}{Convex Adversarial Regularisers}
    \cite{mukherjee2020learned} proposed parameterising the Critic using \textbf{Input Convex Neural Networks (ICNNs)}.
...
\begin{frame}{Outlook}
    ...
    \begin{itemize}
        \item \textbf{Explainability:} Understanding exactly what features the network uses.
        \item \textbf{Uncertainty Quantification:} Not just $u$, but a probability map $P(u|f)$.
        \item \textbf{Posterior Sampling:} Using Diffusion Models (Score-Based Generative Models) to sample from the posterior.
    \end{itemize}
