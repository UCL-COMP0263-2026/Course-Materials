...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{../Common_Images/pnp/smoothed-marginal-likelihood.png}
    \caption{Illustration of the convolution of a complex probability prior with a Gau\ss ian noise kernel resulting in a smoothed marginal likelihood. This diagram was created with Nano Banana Pro.}
    \label{fig:smoothed_marginal_likelihood}
\end{figure}

\begin{rem}\label{rem:marlikconv}
Notice that the marginal likelihood $p_\sigma(y) = \int p(y|x)p(x) \, dx$ is mathematically equivalent to the convolution of the clean prior $p(x)$ with the Gau\ss ian noise distribution $\mathcal{N}(0, \sigma^2 I)$. Therefore, in the context of Gau\ss ian noise, the marginal likelihood of the data is synonymous with a ``smoothed prior''.
\end{rem}
...
\begin{enumerate}
    \item $\nabla_u \log p(f^\delta | u)$: The \emph{likelihood score}. This pushes the image to match the physical measurements. If our data fidelity assumes Gau\ss ian noise, this is exactly the negative gradient of our data term: $-\nabla_u F(Ku, f^\delta)$.
    \item $\nabla_u \log p(u)$: The \emph{prior score}. This pushes the image to look like a natural, clean image. 
\end{enumerate}

Here, however, we encounter a theoretical hurdle. For natural images, the exact prior $p(u)$ is confined to a highly complex, lower-dimensional manifold. If $u$ is even slightly noisy, $p(u) \approx 0$, making the exact prior score $\nabla_u \log p(u)$ undefined or numerically unstable almost everywhere.

Modern \textbf{Score-Based Generative Models} bypass this issue by instead of using the intractable clean prior $p(u)$, they substitute it with the \textbf{smoothed prior} (or marginal distribution) $p_\sigma(u)$, which is the true prior perturbed by Gau\ss ian noise of variance $\sigma^2$ as explained in Remark \ref{rem:marlikconv}. 
...
Because the noise smooths out the probability mass, $p_\sigma(u) > 0$ everywhere, ensuring the score vector field is well-behaved and points smoothly towards the data manifold from any location in the space. Crucially, as we proved previously with culmination in Equation \eqref{eq:score_denoiser}, Tweedie's formula provides exactly this smoothed score via our denoiser, i.e.,
\begin{align*}
    \nabla_u \log p_\sigma(u) = \frac{D_\sigma(u) - u}{\sigma^2} \, .
\end{align*}
By replacing the exact prior score with the smoothed prior score, the Langevin dynamics update becomes practically computable. Substituting these components back into \eqref{eq:langevin_base}, the final sampling update reads
\begin{align}
