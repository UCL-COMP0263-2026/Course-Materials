\chapter{Unrolling algorithms: from iterations to architecture}\label{ch:unrolling}
...
Before introducing first order methods for non-smooth optimisation, let us briefly recall the classical first-order method for smooth convex optimisation. Consider the problem
\begin{align}
\min_{u \in \mathcal{U}} E(u) \, , \label{eq:gd_problem}
\end{align}
where $E \colon \mathcal{U} \to \mathbb{R}$ is convex and continuously differentiable. A minimiser $u^*$ satisfies the first-order optimality condition $0 = \nabla E(u^*)$, and the basic gradient descent iteration reads
\begin{align}
u^{k+1} = u^k - \tau \nabla E(u^k) \, , \label{eq:gradient_descent}
\end{align}
with step size $\tau > 0$. Under standard smoothness assumptions (e.g., Lipschitz continuity of $\nabla E$), this method converges to a minimiser for an appropriate choice of $\tau$.

\paragraph{Bregman proximal derivation}
The gradient descent update \eqref{eq:gradient_descent} can be obtained by a Bregman proximal point method. Consider the iteration
\begin{align*}
u^{k+1} = \argmin_{u \in \mathcal{U}} \left\{ E(u) + D_J(u, u^k) \right\} \, ,
\end{align*}
where the Bregman distance is $D_J(u, v) = J(u) - J(v) - \langle \nabla J(v), u - v \rangle_\mathcal{U}$ as defined in Definition \ref{defi:bregman} and we choose
\begin{align*}
J(u) = \frac{1}{2\tau}\|u\|_\mathcal{U}^2 - E(u) \, .
\end{align*}
Then $\nabla J(u) = \frac{1}{\tau}u - \nabla E(u)$, and a direct expansion yields
\begin{align*}
E(u) + D_J(u, u^k)
&= E(u) + J(u) - J(u^k) - \langle \nabla J(u^k), u - u^k \rangle_\mathcal{U} \, , \\
&= E(u^k) + \langle \nabla E(u^k), u - u^k \rangle_\mathcal{U} + \frac{1}{2\tau}\|u - u^k\|_\mathcal{U}^2 \, ,
\end{align*}
which is the standard quadratic model of $E$ at $u^k$ with step size $\tau$. The first-order optimality condition of this function therefore gives
\begin{align*}
0 = \frac{1}{\tau}(u^{k+1} - u^k) + \nabla E(u^k)
\quad \Longleftrightarrow \quad
u^{k+1} = u^k - \tau \nabla E(u^k) \, ,
\end{align*}
which recovers \eqref{eq:gradient_descent}.

When $E$ is convex but non-smooth, the gradient $\nabla E$ no longer exists. One can replace it by a subgradient $g^k \in \partial E(u^k)$ (cf. Definition \ref{defi:subdiff}) to obtain the subgradient method,
\begin{align}
u^{k+1} = u^k - \tau_k g^k \, , \quad g^k \in \partial E(u^k) \, .
\end{align}
However, convergence requires diminishing step sizes $\{\tau_k\}$, and the resulting rates are typically slow compared to smooth gradient descent.

Many objectives of interest in inverse problems, such as the LASSO and ROF models (Examples \ref{exm:lasso} and \ref{exm:rof_inv}), are overall non-smooth. Consequently, standard gradient descent is not directly applicable, and subgradient descent is often impractical due to its slow convergence. This motivates the use of proximal algorithms, which are specifically designed for such non-smooth convex objectives.
...
To build complex neural network layers using proximal operators, we must understand how these operators behave under summation when the overall function is completely separable.

\begin{thm}[Proximal Map of Separable Functions]\label{thm:prox_separable}\normalfont
...
\end{align*}
\end{thm}
This theorem justifies why non-linear activation functions in Proximal Neural Networks (like the Soft Thresholding operator that follows as the next example) operate element-wise on feature vectors, mirroring standard activations used in neural networks.
...
\end{exm}
Before we proceed to computational methods that utilise proximal operators, let us take a look at closed-form proximal maps for a special case of affine composition.
...
You may wonder why we cannot apply Theorem \ref{thm:prox_affine} to the ROF problem, despite $\operatorname{TV}(u) \approx \| \nabla u \|_1$, which is an affine composition of $\| \cdot \|_1$ and $\nabla$. The problem is that the operator $\nabla$ does not satisfy the necesary condition for Theorem \ref{thm:prox_affine}, i.e., $\nabla \nabla^\ast = -\nabla \operatorname{div} \neq \gamma I$. This is why we will resort to numerical approaches instead.
...
As discussed in the literature (e.g., \cite{benning2018modern}) and similar to the derivation of gradient descent as a Bregman proximal method, PGD can also be interpreted as a Bregman proximal method. If we define the Bregman distance generating function $J(u) = \frac{1}{2\tau}\|u\|_\mathcal{U}^2 - L(u)$, then PGD corresponds to the iteration
\begin{align*}
