\chapter{From hand-crafted to data-driven: learning regularisations}\label{ch:data-driven}
...
The task-centric approach to learning optimal parameters can be naturally formulated as a \emph{bilevel optimisation} problem. We obtain two nested optimisation problems. The lower-level problem is the variational reconstruction from \eqref{eq:var_reg} for fixed parameters, and the upper-level problem minimises a task loss over training data. This mirrors the empirical-risk viewpoint in \eqref{eq:dnn-empirical-risk-minimisation}, but now with a physics-constrained reconstruction map inside the objective.
...
\begin{enumerate}
    ...
    \item \textbf{Non-differentiability}: To solve the upper-level problem with gradient-based methods, we need derivatives with respect to $\theta$, which in turn requires differentiating the solution operator $R_\theta$. To see this explicitly, we consider a single sample, choose $\mathcal{L}_{\text{task}} = \frac12 \|\cdot\|^2$, and specialise to a scalar parameter $\theta = \alpha$, i.e.
    \begin{align*}
    ...
    To compute the derivative $\frac{d}{d\alpha} R_{\alpha}(f^\delta)$, we often rely on the \href{https://en.wikipedia.org/wiki/Implicit_function_theorem}{\emph{Implicit Function Theorem (IFT)}}. From \eqref{eq:bilevel_lower}, for fixed data $f^\delta$ and parameter $\theta$, we define
    \begin{align*}
        E_\theta(u; f^\delta) := F(Ku, f^\delta) + J_\theta(u) \, .
    \end{align*}
    In the scalar case $\theta=\alpha$, the solution $u_\alpha = R_\alpha(f^\delta)$ is characterised by
    \begin{align*}
        \nabla_u E_\alpha(u_\alpha; f^\delta) = 0 \, .
    \end{align*}
    If the Hessian $\nabla_{uu}^2 E_\alpha(u_\alpha; f^\delta)$ is invertible (typically ensured by strict convexity of the lower-level problem), the IFT gives local differentiability of the map $\alpha \mapsto u_\alpha$. Differentiating the optimality condition yields
    \begin{align*}
        \frac{d}{d\alpha} \left( \nabla_u E_\alpha(u_\alpha; f^\delta) \right)
        = \nabla_{uu}^2 E_\alpha(u_\alpha; f^\delta) \frac{du_\alpha}{d\alpha}
        + \nabla_{\alpha u}^2 E_\alpha(u_\alpha; f^\delta) = 0 \, .
    \end{align*}
    ...
    \begin{exm}[Gradient computation for Tikhonov Regularisation]\label{exm:bilevel_tikhonov}\normalfont
    ...
    Suppose the lower-level problem \eqref{eq:bilevel_lower} is continuously differentiable with respect to $\theta$ (respectively $\alpha$), as in Example \ref{exm:bilevel_tikhonov}. Then we can seek $\hat\alpha$ via projected gradient descent, i.e.
    \begin{align*}
